...
mod tests {
    ...
    /// Drive the fitted box for `secs` with the firmware's own `grant_tracking_current`,
    /// the car ramping toward its grant (contactor lag). Returns the car's draw so a
    /// caller can interrupt the loop and resume it.
    fn closed_loop(b: &mut BoxSim, target: f32, mut car: f32, secs: u32) -> f32 {
        use crate::charge::control::{grant_tracking_current, GrantControlInputs};
        for _ in 0..(secs / 5) {
    ...
    #[test]
    fn the_deficit_report_never_opens_a_cold_session_at_the_floor() {
        // Live 2026-07-09, the reason the kick exists: at target 6 the deficit report
        // `max − target` = 10 has the box compute an opening of ceil(16 − 10) = 6 ≤
        // start_threshold 8, so it refuses forever — the stuck-B stall.
        let mut b = BoxSim::new(params()); // steady-state regime: start_threshold 8
        for _ in 0..40 {
            b.tick(5.0, Ampere(10.0), Ampere(0.0));
        }
        assert!(!b.charging());
        assert_eq!(b.lb(), Ampere(0.0));
    }
    ...
    #[test]
    fn the_controller_opens_a_cold_session_at_the_floor() {
        // The full-offer kick reports 0 → opening ceil(16) = 16 > 8 → the box opens; the
        // pin then takes over and the loop settles to a valid session at the floor.
        let mut b = BoxSim::new(params());
        closed_loop(&mut b, 6.0, 0.0, 200);
        assert!(
            b.charging(),
            "the kick opens the cold session the box otherwise refuses"
        );
        assert!(b.lb().0 >= 6.0, "and the loop holds a valid ≥6 A grant");
    }

    #[test]
    fn a_session_that_ended_without_a_pause_reopens() {
        let mut b = BoxSim::new(params());
        let car = closed_loop(&mut b, 6.0, 0.0, 200);
        assert!(b.charging(), "the cold session opens");

        // Something outside the control layer ends the session (car done, unplugged): the
        // box cuts. Nothing pauses the controller — enable, target and both feeds stay
        // valid, so the pause gates never fire and a latch would never re-arm.
        b.tick(5.0, Ampere(20.0), Ampere(car));
        assert!(!b.charging());

        // Live 2026-07-10: the controller remembered the earlier grant, served the deficit
        // report forever, and the box never reopened. A session end is a cold start again.
        closed_loop(&mut b, 6.0, 0.0, 200);
        assert!(b.charging(), "the controller must reopen the session");
    }
}
